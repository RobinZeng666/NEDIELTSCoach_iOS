...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.backgroundColor = RGBACOLOR(39, 182, 149, 1);
        [self initAttributes];
    }
    return self;
}
...
#define kClosedIndicatorWidth 106
- (void)initAttributes
{
...
{
//    self.displayLabel = [[RMDisplayLabel alloc] init ];
//    self.displayLabel.frame = CGRectMake(100/2, 65/2-15, 60, 30);
//    self.displayLabel.backgroundColor = [UIColor clearColor];
//    self.displayLabel.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:16.0f];
//    self.displayLabel.text = @"0";
//    self.displayLabel.textColor = [UIColor redColor];
//    self.displayLabel.textAlignment = NSTextAlignmentCenter;
//    [self addSubview:self.displayLabel];
    
    self.displayLabel = [[RMDisplayLabel alloc] init];//WithFrame:CGRectMake((kClosedIndicatorWidth - 80)/2, (kClosedIndicatorWidth- 30)/2, 80, 30)
    self.displayLabel.backgroundColor = [UIColor clearColor];
    self.displayLabel.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:16.0f*AUTO_SIZE_SCALE_X];
    self.displayLabel.text = @"0";
    self.displayLabel.textColor = [UIColor redColor];
    self.displayLabel.textAlignment = NSTextAlignmentCenter;
    [self addSubview:self.displayLabel];
    
    WS(this_RmdIndeicator);
    [self.displayLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(this_RmdIndeicator);
        make.centerY.mas_equalTo(this_RmdIndeicator);
        make.size.mas_equalTo(CGSizeMake(kClosedIndicatorWidth, 30));
    }];
}
...
//    }
//    if ([titleLabelText isEqualToString:@""]) {
//        //重新设置self.displayLabel的坐标
//        self.displayLabel.frame = CGRectMake( 40/2, 65/2-15, 60, 30);
//        self.displayLabel.font = [UIFont systemFontOfSize:16.0f*AUTO_SIZE_SCALE_X];
//    }
//    
//}
...
- (void)loadIndicator
{
    ...
    CGPoint center = CGPointMake(kClosedIndicatorWidth/2, kClosedIndicatorWidth / 2);
    ...
    _animatingLayer.lineCap = kCALineCapRound;
...
- (void)drawRect:(CGRect)rect
{
    ...
    else if (_type == kRMClosedIndicator)
    {
        ...
        CGPoint center = CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2);
        UIBezierPath *coverPath = [UIBezierPath bezierPath]; //empty path
        [coverPath setLineWidth:_coverWidth/3];
        [coverPath addArcWithCenter:center radius:radius startAngle:0 endAngle:2*M_PI clockwise:YES]; //add the arc
        [_closedIndicatorBackgroundStrokeColor set];
        [coverPath setLineWidth:self.coverWidth/3];
        [coverPath stroke];
    }
}
