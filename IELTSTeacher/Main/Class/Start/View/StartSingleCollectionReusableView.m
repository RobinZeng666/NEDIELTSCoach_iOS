//
//  StartSingleCollectionReusableView.m
//  IELTSTeacher
//
...
@interface StartSingleCollectionReusableView ()
...
@implementation StartSingleCollectionReusableView
...
- (void)_initView
{
    ...
    [self.bottomLine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(this_headview.bgView.mas_bottom).with.offset(-1);
        make.left.mas_equalTo(0);
        make.size.mas_equalTo(CGSizeMake(kScreenWidth, 0.5*AUTO_SIZE_SCALE_Y));
    }];
...
- (UIView *)bottomLine
{
    if (!_bottomLine) {
        ...
        _bottomLine.backgroundColor = RGBACOLOR(230, 230, 230, 1.0);
    }
    return _bottomLine;
}
