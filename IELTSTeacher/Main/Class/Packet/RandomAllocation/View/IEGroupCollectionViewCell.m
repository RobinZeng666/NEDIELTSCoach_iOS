...
@interface IEGroupCollectionViewCell ()
@property (nonatomic,assign) BOOL isMasConstraint;
@end

@implementation IEGroupCollectionViewCell
...
- (void)layoutSubviews
{
    ...
    CHECK_DATA_IS_NSNULL(self.randomModel.IconUrl, NSString);
    CHECK_STRING_IS_NULL(self.randomModel.IconUrl);
    if (![self.randomModel.IconUrl isEqualToString:@""]) {
        NSString *imgPath = [NSString stringWithFormat:@"%@/%@", BaseUserIconPath, self.randomModel.IconUrl];
    ...
    CHECK_STRING_IS_NULL(self.randomModel.sName);
    self.nameLabel.text = self.randomModel.sName;
}
