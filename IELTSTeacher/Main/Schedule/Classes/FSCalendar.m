...
#define kWeekHeight roundf(self.fs_height/7)
...
@property (strong, nonatomic) NSMutableDictionary        *eventBackgroundColors;
...
//    NSArray *weekSymbols = [[NSCalendar fs_sharedCalendar] weekdaySymbols];
    ...
    for (int i = 0; i < weekSymbols.count; i++) {
        ...
        NSString *weeksString =  NSLocalizedString(weekSymbols[i],nil); //[NSString stringWithFormat:@"%@",weekSymbols[i]] ;

        weekdayLabel.text = weeksString;
    ...
    lineLabel.backgroundColor = RGBACOLOR(200, 200, 200, 1.0);
    [self addSubview:lineLabel];
    UILabel *lineLabel2 = [[UILabel alloc]initWithFrame:CGRectZero];
    lineLabel2.backgroundColor = RGBACOLOR(200, 200, 200, 1.0);
    ...
    _backgroundColors[@(FSCalendarCellStateHasEvent)]    = RGBACOLOR(210, 210, 210, 1);

    _titleColors = [NSMutableDictionary dictionaryWithCapacity:4];
    _titleColors[@(FSCalendarCellStateNormal)]      = [UIColor darkGrayColor];
    ...
    _titleColors[@(FSCalendarCellStateHasEvent)]    = [UIColor whiteColor];
    ...
    _subtitleColors[@(FSCalendarCellStateHasEvent)]    = [UIColor whiteColor];
    
    
//    _eventBackgroundColors = [NSMutableDictionary dictionaryWithCapacity:4];
//    _eventBackgroundColors[@(FSCalendarCellStateNormal)]      = [UIColor clearColor];
//    _eventBackgroundColors[@(FSCalendarCellStateSelected)]    = kBlue;
//    _eventBackgroundColors[@(FSCalendarCellStateDisabled)]    = [UIColor clearColor];
//    _eventBackgroundColors[@(FSCalendarCellStatePlaceholder)] = [UIColor clearColor];
//    _eventBackgroundColors[@(FSCalendarCellStateToday)]       = kPink;
//    _eventBackgroundColors[@(FSCalendarCellStateHasEvent)]    = [UIColor lightGrayColor];
...
-(void)layoutSubviews
{
    ...
    self.lineLabel.frame = CGRectMake(0, kWeekHeight, self.fs_width, 0.5);
    self.lineLabel2.frame = CGRectMake(0, self.fs_height-1, self.fs_width, 0.5);
    _collectionView.frame = CGRectMake(0, kWeekHeight+1, self.fs_width, self.fs_height-kWeekHeight-1);
...
//    cell.eventBackGroundColors = self.eventBackgroundColors;
    ...
    cell.hasEventCount      = [self hasEventCountForDate:cell.date];
    [cell configureCell];
    return cell;
}
...
- (void)setEventBackGroundColors:(UIColor *)eventBackGroundColors
{
    if (eventBackGroundColors) {
        _eventBackgroundColors[@(FSCalendarCellStateHasEvent)] = eventBackGroundColors;
    }else
    {
       [_eventBackgroundColors removeObjectForKey:@(FSCalendarCellStateHasEvent)];
    }
    [self reloadData];
}
- (UIColor *)eventBackGroundColors
{
    return _eventBackgroundColors[@(FSCalendarCellStateHasEvent)];
}
...
- (NSInteger)hasEventCountForDate:(NSDate *)date
{
    if (_dataSource && [_dataSource respondsToSelector:@selector(calendar:hasEventCountForDate:)]) {
        return  [_dataSource calendar:self hasEventCountForDate:date];
    }
    return 0;
}
